#include "wifi_provisioning_driver.h"
    ...
    if (event_base == WIFI_PROV_EVENT) {
        switch (event_id) {
            case WIFI_PROV_START:
                ESP_LOGI("provisioning", "Provisioning started");
                break;
            case WIFI_PROV_CRED_RECV: {
                wifi_sta_config_t *wifi_sta_cfg = (wifi_sta_config_t *)event_data;
                ESP_LOGI("provisioning", "Received Wi-Fi credentials"
                         "\n\tSSID     : %s\n\tPassword : %s",
            ...
            case WIFI_PROV_CRED_FAIL: {
                ...
                if (retries >= CONFIG_EXAMPLE_PROV_MGR_MAX_RETRY_CNT) {
                    ESP_LOGI("provisioning", "Failed to connect with provisioned AP, reseting provisioned credentials");
        ...
        ESP_LOGI("provisioning", "Connected with IP Address:" IPSTR, IP2STR(&event->ip_info.ip));
        /* Signal main application to continue execution */
        xEventGroupSetBits(wifi_event_group, WIFI_CONNECTED_EVENT);
    } else if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_DISCONNECTED) {
        ESP_LOGI("provisioning", "Disconnected. Connecting to the AP again...");
        esp_wifi_connect();
    }
}
...
{
    if (inbuf) {
        ESP_LOGI("provisioning", "Received data: %.*s", inlen, (char *)inbuf);
    }
    char response[] = "SUCCESS";
    *outbuf = (uint8_t *)strdup(response);
    if (*outbuf == NULL) {
        ESP_LOGE("provisioning", "System out of memory");
...
void initalize_provisioning(void)
{
    ...
    if (!provisioned) {
        ESP_LOGI("provisioning", "Starting provisioning");
    ...
    // while (1) {
    //     ESP_LOGI("provisioning", "Hello World!");
